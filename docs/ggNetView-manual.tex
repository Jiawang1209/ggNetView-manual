...
\CommentTok{\# Raw ASV/OTU abundance table (rows = ASVs/OTUs, cols = samples)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Rarefied ASV/OTU table (sequencing depth normalised by rarefaction)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Rarefied + converted to relative abundance — most common downstream input}
...
\CommentTok{\# First column must be the ASV/OTU ID (matching rownames of the abundance tables).}
...
\CommentTok{\# Internally: (1) optional transform {-}\textgreater{} (2) correlation {-}\textgreater{} (3) p{-}value adjustment}
\CommentTok{\# {-}\textgreater{} (4) edge filtering {-}\textgreater{} (5) community detection {-}\textgreater{} (6) attach taxonomy.}
\NormalTok{graph\_obj }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_mat}\NormalTok{(}
  ...
  \AttributeTok{transfrom.method =} \StringTok{"none"}\NormalTok{,             }\CommentTok{\# input is already relative abundance}
...
\CommentTok{\# Inspect node/edge counts and the columns now available on nodes:}
...
\CommentTok{\# \textasciigrave{}df\textasciigrave{}: edge list — typically 3 columns: node1 | node2 | weight (optional)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Build a tbl\_graph from an edge{-}list data.frame (no correlation step needed —}
\CommentTok{\# edges are taken as{-}is from \textasciigrave{}df\textasciigrave{}). Community detection still runs on the result.}
\NormalTok{graph\_obj\_from\_df }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_df}\NormalTok{(}
  ...
  \AttributeTok{node\_annotation =}\NormalTok{ node\_annotation,   }\CommentTok{\# per{-}node metadata (joined by name)}
  \AttributeTok{directed        =}\NormalTok{ F,                 }\CommentTok{\# undirected network}
  \AttributeTok{module.method   =} \StringTok{"Fast\_greedy"}\NormalTok{,     }\CommentTok{\# community detection}
  \AttributeTok{top\_modules     =} \DecValTok{15}\NormalTok{,                }\CommentTok{\# keep top{-}15 modules; rest {-}\textgreater{} "Others"}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Same as above, but without any node annotation table.}
\CommentTok{\# Useful when you only have an edge list and no extra metadata to attach.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Like ppi\_example, but \textasciigrave{}annotation\textasciigrave{} already contains a \textasciigrave{}Modularity\textasciigrave{} column}
\CommentTok{\# (i.e. module assignments computed elsewhere — we will NOT recompute them).}
...
\FunctionTok{head}\NormalTok{(node\_annotation)   }\CommentTok{\# note: must contain a \textasciigrave{}Modularity\textasciigrave{} column}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# When you already have pre{-}computed module assignments and want to keep them}
\CommentTok{\# (e.g. from a previous WGCNA run, expert curation, biological prior),}
\CommentTok{\# use the \textasciigrave{}\_module\textasciigrave{} variant — it skips internal community detection and uses}
\CommentTok{\# the Modularity column from \textasciigrave{}node\_annotation\textasciigrave{} directly.}
\NormalTok{graph\_obj\_from\_module }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_module}\NormalTok{(}
  \AttributeTok{df              =}\NormalTok{ df,}
  \AttributeTok{node\_annotation =}\NormalTok{ node\_annotation,   }\CommentTok{\# MUST contain \textasciigrave{}Modularity\textasciigrave{} column}
  \AttributeTok{directed        =}\NormalTok{ F,}
  \AttributeTok{top\_modules     =} \DecValTok{15}\NormalTok{,}
  \AttributeTok{seed            =} \DecValTok{1115}
\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Build a tbl\_graph from a precomputed adjacency matrix.}
\CommentTok{\# Community detection is run internally (Fast\_greedy here) because we have}
\CommentTok{\# weights but no module assignments yet.}
...
\CommentTok{\# Here we just (for demonstration) treat the Kingdom column as the module label;}
\CommentTok{\# in real use, this should be your actual module assignments.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# \textasciigrave{}\_module\textasciigrave{} variant for adjacency input: REQUIRES \textasciigrave{}node\_annotation\textasciigrave{} to contain}
\CommentTok{\# a \textasciigrave{}Modularity\textasciigrave{} column. Skips community detection and uses your provided}
\CommentTok{\# module assignments directly.}
\NormalTok{graph\_obj\_adj\_module }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_adj\_mat\_module}\NormalTok{(}
  \AttributeTok{adjacency\_matrix =}\NormalTok{ adjacency\_matrix\_example,}
  \AttributeTok{node\_annotation  =}\NormalTok{ tax\_tab\_module,   }\CommentTok{\# MUST contain \textasciigrave{}Modularity\textasciigrave{} column}
  \AttributeTok{top\_modules      =} \DecValTok{15}\NormalTok{,}
  \AttributeTok{seed             =} \DecValTok{1115}
\NormalTok{)}
\end{Highlighting}
...
\CommentTok{\# (no within{-}mat1 / within{-}mat2 edges), then thresholded, modularised, joined}
\CommentTok{\# with node annotation.}
\NormalTok{graph\_obj\_double\_mat }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_double\_mat}\NormalTok{(}
  \AttributeTok{mat1            =}\NormalTok{ BASV\_tab,          }\CommentTok{\# set A (e.g. bacteria)}
  \AttributeTok{mat2            =}\NormalTok{ FASV\_tab,          }\CommentTok{\# set B (e.g. fungi)}
  \AttributeTok{module.method   =} \StringTok{"Fast\_greedy"}\NormalTok{,}
...
\CommentTok{\# First build a native igraph object directly from the edge list.}
\CommentTok{\# This is what users typically already have when they come from an igraph workflow.}
...
\begin{verbatim}
## IGRAPH 4da318a UNW- 100 200 -- 
## + attr: name (v/c), group (v/c), weight (e/n)
## + edges from 4da318a (vertex names):
...
\CommentTok{\# (1) Build per{-}method networks on the SAME matrix.}
...
\CommentTok{\# Spearman correlation through psych::corr.test — baseline.}
\NormalTok{g\_cor   }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_mat}\NormalTok{(mat, }\AttributeTok{method =} \StringTok{"cor"}\NormalTok{,   }\AttributeTok{cor.method =} \StringTok{"spearman"}\NormalTok{,}
                                \AttributeTok{proc =} \StringTok{"BH"}\NormalTok{, }\AttributeTok{r.threshold =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{p.threshold =} \FloatTok{0.05}\NormalTok{,}
                                \AttributeTok{module.method =} \StringTok{"Fast\_greedy"}\NormalTok{, }\AttributeTok{seed =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{g\_wgcna }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_mat}\NormalTok{(mat, }\AttributeTok{method =} \StringTok{"WGCNA"}\NormalTok{, }\AttributeTok{cor.method =} \StringTok{"spearman"}\NormalTok{,}
                                \AttributeTok{proc =} \StringTok{"BH"}\NormalTok{, }\AttributeTok{r.threshold =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{p.threshold =} \FloatTok{0.05}\NormalTok{,}
                                \AttributeTok{module.method =} \StringTok{"Fast\_greedy"}\NormalTok{, }\AttributeTok{seed =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Hmisc::rcorr — yet another correlation engine, slightly different tie/NA behaviour.}
\NormalTok{g\_hmisc }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_mat}\NormalTok{(mat, }\AttributeTok{method =} \StringTok{"Hmisc"}\NormalTok{, }\AttributeTok{cor.method =} \StringTok{"spearman"}\NormalTok{,}
                                \AttributeTok{r.threshold =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{p.threshold =} \FloatTok{0.05}\NormalTok{,}
                                \AttributeTok{module.method =} \StringTok{"Fast\_greedy"}\NormalTok{, }\AttributeTok{seed =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# Bundle the per{-}method matrices into a named list. The names ("cor", "wgcna",}
\CommentTok{\# "hmisc") are kept as method labels in subsequent reports.}
...
\CommentTok{\# then min{-}max rescales to [{-}1, 1]. Robust to across{-}method weight{-}scale differences.}
\NormalTok{g\_borda }\OtherTok{\textless{}{-}} \FunctionTok{build\_graph\_from\_consensus}\NormalTok{(}
  \AttributeTok{adj\_list              =}\NormalTok{ adj\_list,}
  \AttributeTok{method                =} \StringTok{"rank\_fusion"}\NormalTok{,}
  \AttributeTok{rank\_fusion\_algorithm =} \StringTok{"borda"}\NormalTok{,}
  \AttributeTok{threshold             =} \FloatTok{0.5}\NormalTok{,        }\CommentTok{\# final |w| cutoff applied to the fused matrix}
  \AttributeTok{top\_modules           =} \DecValTok{5}\NormalTok{,}
